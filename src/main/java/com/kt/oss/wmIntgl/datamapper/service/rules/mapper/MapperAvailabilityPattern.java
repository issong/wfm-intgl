 ...
 * @see
 */
public class MapperAvailabilityPattern extends RulesMapper {

	/**
	 * AvailabilityPattern 항목의 데이터를 적재한다.
	 * 
	 * @param drls
	 * @return
	 * @throws Exception
	...
	public List<Object> generate(_Rules drls, IntglDataVo intglVo,
		...
		for (Object order : intglVo.getSourceData()) {
			if (order instanceof WMAppointmentOrder) {
				List<WMAvailabilityPattern> availabilityPatterns = ((WMAppointmentOrder) order)
						.getAvailabilityPatterns();
				for (WMAvailabilityPattern availabilityPattern : availabilityPatterns) {
					executeGenerate(drls, order, availabilityPattern, intglVo,
	...
	protected Object getTargetObject() {
		return new ObjectFactory().createDsScheduleDataAvailabilityPattern();
	}
}

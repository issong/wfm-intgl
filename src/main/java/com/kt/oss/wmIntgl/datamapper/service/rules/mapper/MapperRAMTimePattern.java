 ...
 * @see
 */
public class MapperRAMTimePattern extends RulesMapper {

	/**
	 * RAMTimePattern 항목의 데이터를 적재한다.
	 * 
	 * @param drls
	 * @return
	 * @throws Exception
		...
		for (Object order : intglVo.getSourceData()) {
			...
			if (tpRogMap.containsKey(wmResourceHolidayInfo.getHolidayTcode()))
				executeGenerate(drls, order, intglVo, rulesVo, rulesDB,
						ollehMapService,
						tpRogMap.get(wmResourceHolidayInfo.getHolidayTcode()));
		}
	...
	protected Object getTargetObject() {
		return new ObjectFactory().createDsModellingRAMTimePattern();
	}
}

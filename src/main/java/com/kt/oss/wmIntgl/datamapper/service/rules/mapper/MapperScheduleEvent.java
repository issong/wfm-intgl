 ...
 * @see
 */
public class MapperScheduleEvent extends RulesMapper {

	/**
	 * ScheduleEvent 항목의 데이터를 적재한다.
	 * 
	 * @param drls
	 * @return
	 * @throws Exception
	...
	protected Object getTargetObject() {
		return new ObjectFactory().createDsScheduleDataScheduleEvent();
	}
	...
	protected boolean isEnableSourceObject(Object... objs) {
		for (Object element : objs) {
			if (element instanceof WMWorkOrder) {
				...
				if (data.getBt360LogOnOffWorkerId() == null
						|| data.getBt360LogOnOffWorkerId().trim().length() == 0)
					return false;
			}
			if (element instanceof WMWorkOrderStatus) {
				WMWorkOrderStatus data = (WMWorkOrderStatus) element;
				if (data.getBt360LogOnOffWorkerId() == null
						|| data.getBt360LogOnOffWorkerId().trim().length() == 0)
					return false;
			}

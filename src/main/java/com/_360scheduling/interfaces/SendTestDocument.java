
package com._360scheduling.interfaces;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>anonymous complex type에 대한 Java 클래스입니다.
 *
 * <p>다음 스키마 단편이 이 클래스에 포함되는 필요한 콘텐츠를 지정합니다.
 *
 * <pre>
 * &lt;complexType>
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="xml" type="{http://www.w3.org/2001/XMLSchema}string" minOccurs="0"/>
 *         &lt;element name="licenceKey" type="{http://www.w3.org/2001/XMLSchema}string" minOccurs="0"/>
 *         &lt;element name="broadcastId" type="{http://www.w3.org/2001/XMLSchema}string" minOccurs="0"/>
 *         &lt;element name="path" type="{http://www.w3.org/2001/XMLSchema}string" minOccurs="0"/>
 *         &lt;element name="planQuality" type="{http://www.w3.org/2001/XMLSchema}double"/>
 *         &lt;element name="waitMinutes" type="{http://www.w3.org/2001/XMLSchema}int"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 *
 *
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "xml",
    "licenceKey",
    "broadcastId",
    "path",
    "planQuality",
    "waitMinutes"
})
@XmlRootElement(name = "SendTestDocument")
public class SendTestDocument {

    protected String xml;
    protected String licenceKey;
    protected String broadcastId;
    protected String path;
    protected double planQuality;
    protected int waitMinutes;

    /**
     * xml 속성의 값을 가져옵니다.
     *
     * @return
     *     possible object is
     *     {@link String }
     *
     */
    public String getXml() {
        return xml;
    }

    /**
     * xml 속성의 값을 설정합니다.
     *
     * @param value
     *     allowed object is
     *     {@link String }
     *
     */
    public void setXml(String value) {
        this.xml = value;
    }

    /**
     * licenceKey 속성의 값을 가져옵니다.
     *
     * @return
     *     possible object is
     *     {@link String }
     *
     */
    public String getLicenceKey() {
        return licenceKey;
    }

    /**
     * licenceKey 속성의 값을 설정합니다.
     *
     * @param value
     *     allowed object is
     *     {@link String }
     *
     */
    public void setLicenceKey(String value) {
        this.licenceKey = value;
    }

    /**
     * broadcastId 속성의 값을 가져옵니다.
     *
     * @return
     *     possible object is
     *     {@link String }
     *
     */
    public String getBroadcastId() {
        return broadcastId;
    }

    /**
     * broadcastId 속성의 값을 설정합니다.
     *
     * @param value
     *     allowed object is
     *     {@link String }
     *
     */
    public void setBroadcastId(String value) {
        this.broadcastId = value;
    }

    /**
     * path 속성의 값을 가져옵니다.
     *
     * @return
     *     possible object is
     *     {@link String }
     *
     */
    public String getPath() {
        return path;
    }

    /**
     * path 속성의 값을 설정합니다.
     *
     * @param value
     *     allowed object is
     *     {@link String }
     *
     */
    public void setPath(String value) {
        this.path = value;
    }

    /**
     * planQuality 속성의 값을 가져옵니다.
     *
     */
    public double getPlanQuality() {
        return planQuality;
    }

    /**
     * planQuality 속성의 값을 설정합니다.
     *
     */
    public void setPlanQuality(double value) {
        this.planQuality = value;
    }

    /**
     * waitMinutes 속성의 값을 가져옵니다.
     *
     */
    public int getWaitMinutes() {
        return waitMinutes;
    }

    /**
     * waitMinutes 속성의 값을 설정합니다.
     *
     */
    public void setWaitMinutes(int value) {
        this.waitMinutes = value;
    }

}
